...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot,col,features):
    #model.eval()
    total_loss = 0
    total_loss_l1 = 0
    n_samples = 0
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        X = X[:,:,[col]+features[col]].transpose(1,2)
        Y = Y[:,:,col]
        # Bayesian estimation
        num_runs = runs
        ...
        with torch.no_grad():
            for _ in range(num_runs):
                output = model(X).transpose(1,2)[:,:,0]
...
def train(data, X, Y, model, criterion, optim, batch_size,col,features):
    model.train()
    total_loss = 0
    n_samples = 0
    iter = 0
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        model.zero_grad()
        for j in range(1):
            tx = X[:,:,[col]+features[col]].transpose(1,2) #multivariate
            ty = Y[:, :, col] 
            ...
            # Run the model multiple times
            for _ in range(num_runs):
                output = model(tx).transpose(1,2)[:,:,0]
                outputs.append(output)
...
def main(experiment):
    ...
    features = [getFeatures(col,adj) for col in range(142)]
    ...
    for col in range(columns):
        ...
        for q in range(30):
            ...
            #hp
            input_size = 10 #fixed
            hidden_size = hidden_sizes[randrange(len(hidden_sizes))]
            ...
            try:
                ...
                for epoch in range(1, num_epoch + 1):
                    ...
                    if  val_loss < best_rse:
                        ...
                        best_hp=[input_size, hidden_size, output_size, epoch, learning_rate]
    ...
    for col in range(columns):
        ...
        with open('models_m/model'+str(col)+'.pt', 'rb') as f:
            model = torch.load(f)
        test_tensor_col = torch.tensor(Data.test_window[:10, [col]+features[col]], dtype=torch.float32).transpose(0,1)
        forecast[col] = model(test_tensor_col).transpose(0,1)[:,0]
